...
function file(pathname: string[]): Response {
    ...
    return new Response("404!", {headers: {"Content-Type": "application/text"}})
}
...
function getFileURL(pathname: string[]): Response {
    ...
    const encrypted = cryptojs.AES.encrypt(path, pass, cryptojs.enc.Utf8).toString()
    const replaceAll = encrypted.replaceAll("/","-")
    return new Response(domain + "/file/getfile/" + replaceAll, {headers: {"Content-Type": "application/text"}})
}

function getFile(pathname: string[]): Response {
    const replaceAll = pathname[2].replaceAll("-","/")
    const path = cryptojs.AES.decrypt(replaceAll, pass).toString(cryptojs.enc.Utf8)
    if(!fs.existsSync(path)){
        return new Response("404!", {headers: {"Content-Type": "application/test"}})
    }
    ...
    return new Response(file, {headers: {"Content-Type": "application/text"}})
}
